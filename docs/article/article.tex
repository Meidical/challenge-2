...
\section{Experiments}
A total of around 80 experiments with different models and model configurations were made. To facilitate this, MLflow was used for tracking and result reproducibility across model configurations.\cite{Zaharia2018MLflow}

\subsection{Survival Classification}
As for survival classification, several model families were tested. All models were tested in three different configurations: default parameters, tuned and tuned with SMOTE. In addition to that, the best models were also tested using GANs with different combinations of epochs and sample sizes.

First, linear classification models like the logistic regression were tested as a baseline. Tree ensemble models like the Random Forest (RR) and the Extra Trees (ET), and gradient boosting models like the XGBoost (XGB) and the LightGBM (LGBM) were also tested. ADTrees and 
